#ifndef SGEVOL_JSON_CONVERT_HPP_INCLUDED
#define SGEVOL_JSON_CONVERT_HPP_INCLUDED
	...
	sge::parse::json::value const &v)
{
		...
		++i)
	{
		...
		if(current_inner_array.size() != first_outer_vector.size())
			throw fcppt::exception(FCPPT_TEXT("You tried to input a matrix, but the inner dimensions don't match!"));
