...
uniform vec3 gradients[gradient_length] = vec3[](
	vec3( 1.0,0.0,0.0),
	vec3(-1.0,0.0,0.0),
	vec3(0.0, 1.0,0.0),
	vec3(0.0,-1.0,0.0),
...
const float inv_factor = -1.0/(1.0 + N + sqrt(1.0 + N));
const float stretch_factor = 1.0/(1.0 + sqrt(1.0 + N));
const mat3 stretch_m = mat3(1.0) + stretch_factor;
const mat3 inv_m = mat3(1.0) + inv_factor;
...
main()
{
	...
	/*
	float n =
		0.5 * sample(position_interp * 1.0 * scale)
		+ 0.25 * sample(position_interp * 2.0 * scale)
		+ 0.125 * sample(position_interp * 4.0 * scale)
		+ 0.0625 * sample(position_interp * 8.0 * scale);
	*/
	float n = sample(gl_FragCoord.xyz * scale);
	n = 0.5 * n + 0.5;
